...
LeptonPhotonMatcher::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (matchFSR) {
	...
	if( dRMin<0.07 ){
	  ...
	  gRelIso = g->userFloat("fsrPhotonPFIsoChHadPUNoPU03pt02")+ g->userFloat("fsrPhotonPFIsoNHadPhoton03");//(g->chargedHadronIso() + g->photonIso() + g->neutralHadronIso() + g->puChargedHadronIso())/g->pt();
